...
[**Download chapter PDF (.pdf).**](https://drive.usercontent.google.com/u/1/uc?id=1XqQNLGQIcj8VJpMFyub5VrgQXK-8e6pV&export=download)

[**Download chapter notebook (.ipynb).**](https://drive.usercontent.google.com/u/1/uc?id=1ImgGD-MBD1hnJNlW_QhbetoZWOXfP7tm&export=download)
...
<p style='text-align: justify;'>
Since the file is quite large, and this is an instructional course where you may wish to use a GitHub Codespace to practise these examples, we will use an online, remotely-hosted copy of the image. 
</p>
<p style='text-align: justify;'>
...
<p style='text-align: justify;'>
We can see that the .shape attribute reveals that this image has three dimensions, and a total of 256 x 256 x 124 volume pixels (or voxels). To visualise our image, we can plot one slice at a time. The example below shows three different slices, in the transverse direction (from chin to the top of the head). To access an image across the transverse orientation, a single value is selected from the third dimension of the image, as follows. As always, the colons indicate that all data on the remaining two axes is selected and visualised:
</p>
...
What about the other dimensions? We can also plot coronal and sagittal slices.  However take note that, in this case, the respective slices have different pixel resolutions. Let us expand our code even further, to display all three desired viewing planes in our MRI scan, as follows:
...
1. Read the image into your workspace and display it.

2. Extract and isolate each of the colour channels (RGB): Save each colour channel as its own variable, plot the image with only a single colour per subplot, and separately plot the respective histograms of each colour.

3. Create three different binary masks using manually determined thresholds, one for each colour channel. Pick an appropriate threshold based on the figures above and create a masked image by setting the values that correspond to False to zero. Note that you can apply conditions that are either greater than or smaller than a threshold of your choice.

4. Plot the three masks and their corresponding masked images.

5. Using your masks, approximate the relative amounts of synaptophysin, IP3R, and DNA in the image. You can assume `red pixels = synaptophysin`, `green pixels = IP3R`, and `blue pixels = DNA`. Give your results as a percentage of the total pixels. The results will vary depending on the setting of the thresholds. Have a play around with different threshold values and see how they change your results.
...
- Masking isolates pixels whose intensity values are below a certain threshold.
